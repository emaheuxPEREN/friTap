...
// Statically-linked hosts (a Go/Rust binary or a self-contained daemon that links
// OpenSSL/BoringSSL in) keep the standard SSL_* surface in .symtab rather than
// .dynsym. Opting the module into deep symbol resolution lets readAddresses /
// isSymbolAvailable fall back to enumerateSymbols(), so the
// SSL_CTX_set_keylog_callback keylog path resolves and installs instead of
// dropping to the ssl_log_secret byte-pattern scan. This is exports-first: the
// fallback only fills symbols the exports pass MISSED, so it is a no-op on a
// normal shared libssl.so. Mirrors boring_execute_modern
// (agent/tls/platforms/linux/openssl_boringssl_linux.ts).
...
// Same reasoning as boring_execute: a Python built against a statically linked
// libssl exposes SSL_CTX_set_keylog_callback only in .symtab, and without it this
// executor has no keylog path at all.
